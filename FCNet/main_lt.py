...
def compose_gif(save_path, save_file):
    img_paths = os.listdir(save_path)
    print(img_paths)
    gif_images = []
    for path in img_paths:
        gif_images.append(imageio.imread(save_path + path))
    imageio.mimsave(save_file, gif_images,fps=6)

def lottery_experiment(model,x,y,initialization,size_net = 20,threshold = [80], epoch = 400):
    num_classes = 2
    biases = True
    criterion = nn.CrossEntropyLoss()
    ...
    for thresh in threshold:
        ...
        if initialization == 'lottery':
            ...
            print(model_lot)
#             print(model_lot.fc1.weight.data)
#             print(model_lot.fc2.weight.data)
            model_lot, acc = train(model_lot.to(device),criterion,x,y,0.01,epoch,batch_size, num_classes, init=initialization, thre=thresh)
        ...
        if initialization == 'xavier':
            ...
            Xavier = train(Xavier.to(device),criterion,x,y,0.01,epoch,batch_size,num_classes, init=initialization, thre=thresh)
        ...
        if 'gaussian' in initialization:
            ...
            Gaussian = train(Gaussian.to(device),criterion,x,y,0.01,epoch,batch_size,num_classes, init=initialization, thre=thresh)
            plotted_model = Gaussian


        # save
        path_prefix = './' + initialization + str(thresh)
        save_prefix = './figures/' + initialization + str(thresh)
        if not os.path.exists(save_prefix):
            os.makedirs(save_prefix)
        # change_name(path_prefix+'/spline_fc1/')
        # change_name(path_prefix+'/spline_fc2/')
        # change_name(path_prefix+'/spline_fc1_fc2/')
        # compose_gif(path_prefix+'/spline_fc1/', save_file=save_prefix+'/test_fc1.gif')
        # compose_gif(path_prefix+'/spline_fc2/', save_file=save_prefix+'/test_fc2.gif')
        # compose_gif(path_prefix+'/spline_fc1_fc2/', save_file=save_prefix+'/test_fc1_fc2.gif')

        # save model
        model_summary.append(plotted_model)
        acc_list.append(acc)

    return model_summary, acc_list

def run_lottery(size_net, x, y, iters):
    num_classes = 2
    biases = True
    model = Net(size_net,x.size(1),num_classes,biases).to(device)
    sum_acc_list = [0]*6
    for i in range(iters):
        lottery_model, acc_list = lottery_experiment(model, x, y, 'lottery', size_net)
        sum_acc_list = [sum_acc_list[i] + acc_list[i] for i in range(len(acc_list))]
    sum_acc_list = [sum_acc_list[i] / iters for i in range(len(sum_acc_list))]
    print(sum_acc_list)
...
if __name__ == '__main__':
    ...
    x, y = gen_data()
    run_lottery(args.net_size, x, y, args.iters)