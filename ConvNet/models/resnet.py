...
class ResNet(nn.Module):
    ...
    def __init__(self, dataset, block, layers, num_classes=1000, cfg=None):
        super(ResNet, self).__init__()

        in_channel, in_size, num_classes = get_dataset_setting(dataset)
        self.inplanes = 64
        if in_size < 64:
        ...
        self.conv1 = nn.Conv2d(in_channel, 64, kernel_size=kernel_size, stride=stride, padding=padding, bias=False)
...
def resnet18(dataset, pretrained=False, cfg=None, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(dataset, BasicBlock, [2, 2, 2, 2], cfg=cfg, **kwargs)
    if pretrained:
